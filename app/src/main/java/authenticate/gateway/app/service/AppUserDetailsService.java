...
public class AppUserDetailsService {
  ...
  public AppUserDetailsResponse authenticateLogin(
      AppUserDetailsRequest appUserDetailsRequest, String sourceIp)
    ...
    if (appUserDetailsRequest.getUserDetails() == null) {
      appUserDetails =
          getAppUserDetails(
              appUserDetailsRequest.getUsername(), appUserDetailsRequest.getPassword());
    } else {
  ...
  private AppUserDetails getAppUserDetails(String username, String passwordInput)
      throws ServerException, ClientException {
    ...
    if (bCryptPasswordEncoder.matches(passwordInput, appUserDetails.getPassword())) {
      ...
      log.error(
          "Get App User Details Wrong Password: [{}] | [{}]", username, appUserDetails.getId());
      throw new ClientException("Get App User Details Wrong Password");
    }
  }

  private AppUserDetails saveAppUserDetails(AppUserDetails appUserDetails)
      throws ServerException, ClientException {
    AppUserDetails appUserDetailsNew =
  ...
  public String createToken(String username, String sourceIp) throws ServerException {
    try {
      AppUserDetailsRequest tokenClaim =
          AppUserDetailsRequest.builder().username(username).sourceIp(sourceIp).build();
