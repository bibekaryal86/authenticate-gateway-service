...
public class AppUserDetailsConnector {
  ...
  private URI getUri(String endpoint) {
    return UriComponentsBuilder.fromHttpUrl(
            String.format(
                "https://data.mongodb-api.com/app/%s/endpoint/data/beta/action/%s",
  ...
  private HttpEntity<?> getHttpEntity(String username, AppUserDetails appUserDetails) {
    ...
    AppUserDetailsMongo appUserDetailsMongo =
        AppUserDetailsMongo.builder()
            ...
            .filter(
                username == null
                    ? null
                    : AppUserDetailsMongoFilter.builder().username(username).build())
            .build();
  ...
  public AppUserDetails getAppUserDetails(String username) throws ServerException {
    ...
    try {
      ResponseEntity<AppUserDetailsMongo> responseEntity =
          restTemplate.exchange(
              getUri("findOne"),
              HttpMethod.POST,
              getHttpEntity(username, null),
              AppUserDetailsMongo.class);

      log.info(
          "Get App User Details Response: [{}] | [{}]",
    ...
    try {
      ResponseEntity<AppUserDetailsMongo> responseEntity =
          restTemplate.exchange(
              getUri("insertOne"),
              HttpMethod.POST,
              getHttpEntity(null, appUserDetails),
              AppUserDetailsMongo.class);

      log.info(
          "Save App User Details Response: [{}] | [{}]",
          responseEntity.getStatusCode(),
          responseEntity.getBody());
