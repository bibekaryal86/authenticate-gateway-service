...
public class AppUserDetailsController {
    ...
    public ResponseEntity<AppUserDetailsResponse> userAuthenticateLogin(@PathVariable("username") String username,
                                                                        @RequestBody AppUserDetailsRequest appUserDetailsRequest,
                                                                        ServerHttpRequest request) {
        if (validateRequest(username, appUserDetailsRequest)) {
            try {
                return ResponseEntity.ok(appUserDetailsService.authenticateLogin(appUserDetailsRequest, getSourceIp(request)));
            } catch (ClientException exc) {
                return new ResponseEntity<>(AppUserDetailsResponse.builder().build(), HttpStatus.UNAUTHORIZED);
            } catch (ServerException exs) {
                return new ResponseEntity<>(AppUserDetailsResponse.builder().build(), HttpStatus.INTERNAL_SERVER_ERROR);
            }
        }
    ...
    private boolean validateRequest(String username, AppUserDetailsRequest appUserDetailsRequest) {
        log.info("Incoming Request User Authenticate Login: [{}] | [{}] | [{}] | [{}]",
                username, appUserDetailsRequest,
